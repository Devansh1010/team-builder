...
export async function POST(req: NextRequest) {
  try {
    ...
    const groupAccessData = await Group.findById(groupId).select('accessTo members name')

    if (!groupAccessData) {
      return createResponse({ success: false, message: 'Group not found' }, StatusCode.BAD_REQUEST)
    }
    ...
    try {
      ...
      await User.findByIdAndUpdate(user.id, { $pull: { groups: { groupId } } }, { session })
